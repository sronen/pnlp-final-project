...
def train_and_test(training_root, test_root, lem_flag=False):
	'''
	Train on given training corpus and test on given test corpus  
	'''
	...
	training_corpus = nltk.corpus.PlaintextCorpusReader( \
		training_root, '.*\.txt')
	tfidfs_per_doc, idfs = \
		wiki_frequency.calculate_corpus_tf_idf(training_corpus)
	
	# Test
	print "Classifying test corpus..."
	st_time = time.time()
	results = wiki_frequency.batch_classify_gold(test_root, \
		tfidfs_per_doc, idfs, lem_flag)
	print "Classified %d articles from %s in %.03f seconds" % \
		(len(results), test_root, time.time()-st_time)
...
if __name__ == "__main__":	
	...
	results = train_and_test(training_corpus_path, test_root)
	measures = eval_stats(results)
	
	print "Lemmatized:\n"
	print "********"
	num_docs_lem, training_corpus_path_lem = \
		str_corpus_cleaner.create_corpus_files(training_root, '_eval_lem_', lem_flag=True)
	results_lem = train_and_test(training_corpus_path_lem, test_root, lem_flag=True)
	measures = eval_stats(results_lem)
