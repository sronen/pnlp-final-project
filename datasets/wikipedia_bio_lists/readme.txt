...
1) Copy person_wiki.tsv to the same folder as find_wiki_langlinks.py 
 and wiki_article_meta.py, get_article_sizes.py, merge_and_delete.py, and
 remove_article_duplicates.R (usually retrieve/).
...
5) To get sizes, run get_article_sizes.py. Update BIO_LIST_FILE and the Wikipedia API URLs as necessary.
6) Once done, run merge_and_delete.py. Update the four constants for filename and titles as necessary.
7) Once done, run remove_duplicates.R to remove any duplicate entries.
...
Note II:
consider using WPIDs instead of Wikipedia names - may be more stable.